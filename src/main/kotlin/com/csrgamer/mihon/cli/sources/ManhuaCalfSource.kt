...
class ManhuaCalfSource : MangaSource() {
    override val name = "ManhuaCalf"
    private val baseUrl = "https://manhuacalf.com"
    private val httpClient = HttpClient()

    override suspend fun search(query: String, limit: Int): List<Manga> {
        logger.info { "Searching ManhuaCalf for: $query" }
        return try {
            val response = httpClient.get("$baseUrl/?s=$query")
            val html = response.body<String>()
            val doc = Jsoup.parse(html)

            doc.select(".item").take(limit).map { element ->
            ...
            logger.error(e) { "ManhuaCalf search failed" }
            emptyList()
        }
    }
