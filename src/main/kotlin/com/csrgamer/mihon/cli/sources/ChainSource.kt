...
class ChainSource : MangaSource() {
    override val name = "Chain"
    private val baseUrl = "https://chainreader.app"
    private val httpClient = HttpClient()

    override suspend fun search(query: String, limit: Int): List<Manga> {
        logger.info { "Searching Chain for: $query" }
        return try {
            val response = httpClient.get("$baseUrl/search?title=$query")
            val html = response.body<String>()
            val doc = Jsoup.parse(html)

            doc.select(".manga-item").take(limit).map { element ->
            ...
            logger.error(e) { "Chain search failed" }
            emptyList()
        }
    }
